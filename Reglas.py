                            ...
                            for h1 in range(6):
                                if inicial:
                                    if(m1 != m2 or h1 != h2) and (m1 != m3 or h1 != h3) and (m2 != m3 or h2 != h3):
                                        formula = P(m3, d, h3, Nmaterias, Ndias, Nhoras) + P(m2, d, h2, Nmaterias, Ndias, Nhoras) + 'Y'
                                        formula += P(m1, d, h1, Nmaterias, Ndias, Nhoras) + 'Y'
                                        inicial = 0
                                else :
                                
                                    if(m1 != m2 or h1 != h2) and (m1 != m3 or h1 != h3) and (m2 != m3 or h2 != h3):
                                        formula += P(m3, d, h3, Nmaterias, Ndias, Nhoras) + P(m2, d, h2, Nmaterias, Ndias, Nhoras) + 'Y'
                                        formula += P(m1, d, h1, Nmaterias, Ndias, Nhoras) + 'Y' + 'O'
    ...
    for m in range(7):
        for d in range(6):
            if inicial:
                ...
                formula += P(m, d, 2, Nmaterias, Ndias, Nhoras) + 'O'
    formula += '-'
    return formula
#-------------------------------------Regla 4-----------------------------------------------------#
def regla4(i):
